...
void loop(){
  if (ISR_Happened == 1){
   ...
   //print_comma_then_float(PD.dt);
    ...
    Serial.print(",");
    Serial.print(PD.dt,4);

    print_comma_then_int(sum1.read_output());//<--
    print_comma_then_int(PD.input_value);//<-- how are these not the same?
    print_comma_then_int(PD.din);
    print_comma_then_int(PD.din_dt);
