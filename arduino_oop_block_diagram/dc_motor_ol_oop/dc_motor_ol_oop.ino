...
class pulse_block{
 public:
  ...
  float off_time;
  int amp;
  pulse_block(float switch_on_time, float switch_off_time, int Amp){
...
pulse_block u = pulse_block(1, 1.75, 150);
...
void setup(){
   Serial.begin(115200);
   Serial.println("OOP for DC motor v.1.0.0");
...
void loop(){
  if (ISR_Happened == 1){
    ...
    motor_speed = u.get_output(t_sec);
