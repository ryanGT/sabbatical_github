...
class actuator_or_sensor_chooser(my_toplevel_window):
    def __init__(self, parent, dialog_type="actuator", class_list=[], \
                 param_lists_dict={}, defaults_dicts={}, geometry='600x300', max_params=5):        
        if dialog_type.lower() == "actuator":
            title = "Add Actuator Dialog"
            self.dialog_type_int = 0
        else:
            title = "Add Sensor Dialog"
            self.dialog_type_int = 1

        super().__init__(parent, title=title, geometry=geometry)

        self.dialog_type = dialog_type.lower()
        self.class_list = class_list
        self.param_lists_dict = param_lists_dict
        self.defaults_dicts = defaults_dicts
    ...
    def make_widgets(self):
        if self.dialog_type == "actuator":
            main_label = 'Actuators'
            name_label = "Actuator Name"
            class_names = pybd.actuator_class_names
            go_text = "Add Actuator"
        else:
            main_label = 'Sensors'
            name_label = "Sensor Name"
            class_names = pybd.sensor_class_names
            go_text = "Add Sensor"
        ...
        self.main_chooser_combobox['values'] =  class_names
        self.main_chooser_combobox.bind('<<ComboboxSelected>>', self.main_combo_selected)
        
        self.make_label_and_grid_sw(name_label, 2, mycol)
        ...
        self.go_button = self.make_button_and_grid(go_text, 20, mycol, \
    ...
    def main_combo_selected(self, *args, **kwargs):
        ...
        msg = "cannot create actuators if self.parent doesn't have a block diagram"
        assert hasattr(self.parent, "block_diagram"), msg
        assert self.parent.block_diagram is not None, msg
        param_list = pybd.actuator_params_dict[chosen_type]#<--- this is actuator hard coded
        ...
        suggest_name = self.parent.block_diagram.suggest_actuator_name(chosen_type)
